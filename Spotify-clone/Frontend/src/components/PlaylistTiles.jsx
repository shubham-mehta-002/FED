...
export function PlaylistTiles({playlist})
{
    ...
    // console.log('play tile',{playlist})

    return (
        <div key={playlist._id} onClick={()=>navigate(`playlist/${playlist._id}`)} className=" h-1/4 will-change-auto p-1 text-white  m-1 flex flex-row gap-4 hover:bg-customGray hover:cursor-pointer rounded-md">
            ...
            <div className="h-full w-3/5 flex flex-col justify-around ">
                ...
                <div className='text-sm'>{playlist.songs.length} 
                <span> songs</span>
                <span> <button className='hover:cursor-default  border-2 px-1 border-black border-solid bg-customGray rounded-full'>{playlist.visibilityMode}</button></span>
                </div>
            </div>
