...
//     },[])
    ...
    return (
        <div className="playlists-container h-full w-full  flex flex-col justify-start">
            ...
            className=" h-1/4 will-change-auto p-1 text-white  m-1 flex flex-row gap-4 hover:bg-customGray hover:cursor-pointer rounded-md">
                ...
                <div className="h-full w-3/5 flex flex-col justify-around ">
                    ...
                    <div className='text-sm'>{state.likedSongs ? state.likedSongs.length : 0} 
                    <span> songs</span>
        ...
        {/*
            ...
            {/* playlist tiles */}
            {
                state.playlists && state.playlists.map(playlist => <PlaylistTiles key={playlist._id} playlist={playlist}/>)
            }  
        </div>
    );
}
