...
export function SongTiles({_id,thumbnail,name,track,artist,duration, addButton ,playlistId,alreadyInPlaylist})
{
    ...
    // const  [isAddedToPlaylist , addToPlaylist] = useState(false)
    ...
    // state.currentSong && state.currentSong.play()
    ...
    function toggleMusicPlay(){
        ...
        // here  i am checking if there is no current song or user has selected sone another song , then update the new song in the state 
        // state.audio.pause()
        if((!state.currentSong) || ((state.currentSong) && (state.currentSong._id !== _id)))
            dispatch({type:"SET_CURRENT_SONG", payload:{currentSong : songData }})
        else
            state.currentSong._id === _id ? dispatch({type:"TOGGLE_PLAYING" , payload:{songId:_id , isPlaying : state.isPlaying ? false: true}}) : null

    }

    // const togglePlayPause = (e) =>{
    //     // console.log(state.currentSong._id , _id)
    //     // if(state.currentSong._id === _id)
    //     e.stopPropagation();    // Stop event propagation to prevent parent div click
    //     // console.log(state.isPlaying)
    //         dispatch({type:"TOGGLE_PLAYING" , payload:{songId:_id , isPlaying : state.isPlaying ? false: true}})
    // }
        ...
        <div 
            ...
            <div className=" details h-full w-1/2 flex flex-row gap-6 items-center">
                ...
                <div className="h-full w-1/2 flex flex-col justify-center items-center gap-2">
                    <div className="h-1/3 w-full whitespace-nowrap overflow-hidden text-ellipsis  text-xl font-semibold">{name}</div>
                    <div className="h-1/3 w-full whitespace-nowrap overflow-hidden text-ellipsis ">{artist.username}</div>
                </div>
...
// state.currentSong ? state.currentSong._id === _id ? togglePlayPause : null : toggleMusicPlay


 {/* </div>
            <div className=" play h-full w-1/3 flex flex-row items-center justify-end gap-5 ">
                <img className='h-8 w-8' src={isLiked? greenHeart : whiteHeart} />
                <img className='h-8 w-8 mr-5' src={playButtonWhite} />
 </div> */}